...
class LivroDAO
{
	...
	{
		$query = "INSERT INTO livros(nome, isbn, autor, disponibilidade) values('{$livro->getNome()}','{$livro->getIsbn()}', '{$livro->getAutor()}', '{$livro->getDisponibilidade()}')";
	...
	{
		$query = "update livros set nome='{$livro->getNome()}', isbn='{$livro->getIsbn()}', autor='{$livro->getAutor()}', disponibilidade='{$livro->getDisponibilidade()}' where id='{$livro->getId()}'";
		var_dump($query);
		return mysqli_query($this->conexao, $query);
